...
const Certificate = () => {
  const navigate = useNavigate();
    ...
    useEffect(() => {
      ...
      document.title = 'Certificate - API';
    }, []);
    ...
    useEffect(() => {
      ...
      if (socket && !hasJoinedRoom) {
        ...
        Room.join(socket, "certificate", objID);
      ...
      return () => {
        if (hasJoinedRoom) {
          socket.emit("leave-room", "certificate");
        }
        socket.off("message");
        socket.off("redirect");
        socket.off("error");
  ...
  return (
    <Layout title="Certificate - API" activePage="certificate">
      <div>
        <h1>Certificate - API</h1>
        <p>This is the Server management page for Certificate API</p>
      </div>
    </Layout>
  );
};
